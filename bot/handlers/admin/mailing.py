...
from bot.kb.admin import mailing_callback, get_towns_for_mailing
from bot.states.admin import MailingState


@dp.callback_query_handler(IsSuperAdmin(), mailing_callback.filter(action="start"), state="*")
...
@dp.callback_query_handler(IsSuperAdmin(), mailing_callback.filter(action="assing_town"), state=MailingState.add_town)
@session_decorator()
async def mailing_start(cq: types.CallbackQuery, callback_data: dict, state: FSMContext):
