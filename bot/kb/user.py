...
async def get_main_user_menu(current_session, town_id):
    ...
    query = select(Category).filter_by(parent_category_id=None).filter(
        or_(Category.town_id == None, Category.town_id == town_id)).order_by(Category.rating.desc(), Category.id)
